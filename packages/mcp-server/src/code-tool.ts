...
import { ClientOptions } from 'sdk-limes';

const prompt = `Runs JavaScript code to interact with the SDK Limes API.
...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { SDK_LIMES_API_KEY: readEnv('SDK_LIMES_API_KEY') ?? client.apiKey ?? undefined, SDK_LIMES_BASE_URL: readEnv('SDK_LIMES_BASE_URL') ?? client.baseURL ?? undefined };
