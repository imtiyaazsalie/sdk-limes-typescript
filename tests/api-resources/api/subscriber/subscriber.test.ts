...
describe('resource subscriber', () => {
  ...
  test.skip('create: request options and params are passed correctly', async () => {
    ...
    await expect(client.api.subscriber.create({
    accountId: 'accountId',
    address: [{
    addressType: 'POSTAL',
    city: 'city',
  ...
  test.skip('getUser: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.api.subscriber.getUser({ limit: 0, page: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(SDKLimes.NotFoundError);
  });
  ...
  test.skip('search: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.api.subscriber.search({
    AccountId: 'AccountId',
    Active: 'Active',
    AddressId: 'AddressId',
