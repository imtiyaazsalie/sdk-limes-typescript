...
export interface ClientOptions {
  /**
   * Defaults to process.env['SDK_LIMES_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['SDK_LIMES_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['SDK_LIMES_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the SDK Limes API. 
 */
export class SDKLimes {
  apiKey: string | null;
   ...
   * API Client for interfacing with the SDK Limes API.
   *
   * @param {string | null | undefined} [opts.apiKey=process.env['SDK_LIMES_API_KEY'] ?? null]
   * @param {string} [opts.baseURL=process.env['SDK_LIMES_BASE_URL'] ?? https://api.example.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('SDK_LIMES_BASE_URL'),
    apiKey = readEnv('SDK_LIMES_API_KEY') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    this.timeout = options.timeout ?? SDKLimes.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('SDK_LIMES_LOG'), 'process.env[\'SDK_LIMES_LOG\']', this) ?? defaultLogLevel;
  ...
  static SDKLimes = this;
  static DEFAULT_TIMEOUT = 60000 // 1 minute

  static SDKLimesError = Errors.SDKLimesError;
...
SDKLimes.API = ApiapiAPI;
SDKLimes.Health = Health;

export declare namespace SDKLimes {
